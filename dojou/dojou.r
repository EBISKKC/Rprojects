...
vars <- c("deltapH", "exH", "CEC_eff", "CEC_pot", "CationSaturation", "ExCa", "ExMg", "ExK", "ExNa", "Pabsorption")
data_box <- pivot_longer(data, cols = all_of(vars), names_to = "variable", values_to = "value")
p3 <- ggplot(data_box, aes(x = factor(USDA_SoilOrder), y = value, fill = factor(USDA_SoilOrder))) +
